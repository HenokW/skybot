...
module.exports = async (client, options) => {
  ...
  const exemptChannels = (options && options.exemptChannels) || []; // Default Value: Nothingness (None)
  const exemptGuilds = (options && options.exemptGuilds) || []; // Default Value: Nothingness (None)
  ...
  if(exemptChannels.constructor !== Array) throw new Error("exemptChannels must be an array.");
  if(exemptGuilds.constructor !== Array) throw new Error("exemptGuilds must be an array.");
 ...
 client.on("checkMessage", async (message) => {
  ...
  const banUser = async (m, banMsg) => 
  {
    for (var i = 0; i < messageLog.length; i++) 
    {
      ...
      if (user) 
      {
        mutedRole = m.guild.roles.find(role => role.id == '554705282706571274');
        user.addRole(mutedRole).then((member) => 
        {
          setTimeout(() => {user.removeRole(mutedRole);}, 3600000); //1-Hour
          m.channel.send(`You've been automatically muted for **an Hour** due to spam related messaging. If you find this to be an error, please get with a staff member.`);
          m.guild.channels.get('555824593726603265').send(`${m.author}, has been automatically muted for spam related messaging.`);
          return true;
       }).catch(() => 
       {
          m.channel.send(`Oops, seems like i don't have sufficient permissions to punish <@!${message.author.id}>!`);
          return false;
      });
    }
  }
   ...
   const warnUser = async (m, reply) => {
    ...
    if (message.member.roles.some(r => exemptRoles.includes(r.name)) || exemptUsers.includes(message.author.tag) || exemptChannels.includes(message.channel.id) || exemptGuilds.includes(message.guild.id)) return;
