	...
	try
	{
		...
		if(taggedWarnedUser != undefined)
		{
			...
			data =
			{
			  ...
			  botWarning: 0,
			  userWarning: 1,
			  reason: `Warned by ${message.author.username}\n${outputReason}`,
			  staff: `${message.author.username}#${message.author.discriminator}`
			}

			await client.setUserOffenseData.run(data);
			await client.setGuildPointerData.run(offendingUsersWarnFile)

		   	warnPFile.close();
		   	warnOffenseSheet.close();

		   	message.delete().catch(err => {console.log("Recieved an error deleting a warning message. Ignorable, but also unknown.");});
		   	message.channel.send(`<:warning_emote:559030535540834324> **<@${nextWarnArg}>, has been warned.**`);
		   	client.guilds.get(defaultGuild).channels.get(warnLogChannel).send(`<:warning_emote:559030535540834324> <@${nextWarnArg}>, has been warned by ${message.author}`); //Warning message
