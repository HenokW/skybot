...
{//<:warning_emote:559030535540834324>
	...
	if(taggedWarnedUser != undefined)
	{
		...
		data =
		{
		  ...
		  botWarning: 0,
		  userWarning: 1,
		  reason: `Muted by ${message.author.username}\n${outputReason}`,
		...
		client.setGuildPointerData.run(offendingUsersWarnFile)
	   	...
	   	message.delete().catch(err => {console.log("Recieved an error deleting a mute message. Ignorable, but also unknown.");});
	   	taggedWarnedUser.kick(outputReason);
	   	message.channel.send(`<:warning_emote:559030535540834324> <@${nextWarnArg}>, has been muted`);
	   	client.guilds.get(defaultGuild).channels.get(warnLogChannel).send(`<:warning_emote:559030535540834324> <@${nextWarnArg}>, has been kicked by ${message.author}`); //Warning message
