...
async function giveCrate(guild, client, message, args)
{
	...
	if((taggedUser != undefined) && (amount * 0 == 0) && (amount > 0))
	{
		...
		message.channel.send(message.author + " has successfully removed <@" + nextArg + ">, **" + Math.floor(amount * 1) + " Crates!**");
		for(_n = 0; _n < notifier.length; _n++)
		{
			let adminProf = message.guild.members.find(member => member.id == notifier[_n]);
			adminProf.send("========================\n" + message.author.username + "#" + message.author.discriminator + " has successfully removed " + taggedUser.username + "#" + taggedUser.discriminator + ", **" + Math.floor(amount * 1) + " Crates!**\n\n**Guild Name:** " + message.guild.name + "\n**Guild ID:** " + message.guild.id + "\n========================");
		}
