...
const notifier = ['516206994718326795', '148278118170361857']; //We're going to have to send a message to a selected bunch to notify them of an admin/someone gifting crates
...
module.exports.run = async(guild, client, message, args, guildPrefix, guildDate_dir, confi) =>
{
	...
	//We can choose whether or not to disable crate giving, along with other crate stuff in their respected files
	if(!isDisabled)
...
async function giveCrate(guild, client, message, args)
{
	...
	if((taggedUser != undefined) && (amount * 0 == 0) && (amount > 0))
	{
		...
		message.channel.send(message.author + " has successfully given <@" + nextArg + ">, **" + Math.floor(amount * 1) + " Tier II Crates!**");
		for(_n = 0; _n < notifier.length; _n++)
		{
			let adminProf = message.guild.members.find(member => member.id == notifier[_n]);
			adminProf.send("========================\n" + message.author.username + "#" + message.author.discriminator + " has successfully given " + taggedUser.username + "#" + taggedUser.discriminator + ", **" + Math.floor(amount * 1) + " Tier II Crates!**\n\n**Guild Name:** " + message.guild.name + "\n**Guild ID:** " + message.guild.id + "\n========================");
		}
	...
	if(amount < 0)
		message.channel.send(message.author + ", you can't give negative crates.");
