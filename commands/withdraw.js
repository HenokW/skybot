...
async function giveBal(guild, client, message, args)
{
	...
	if((taggedUser != undefined) && (amount * 0 == 0) && (amount > 0))
	{
		...
		message.channel.send(message.author + " has successfully removed <@" + nextArg + ">, **" + Math.floor(amount * 1) + " Quarters!**");
		for(_n = 0; _n < notifier.length; _n++)
		{
			let adminProf = message.guild.members.find(member => member.id == notifier[_n]);
			adminProf.send("========================\n" + message.author.username + "#" + message.author.discriminator + " has successfully removed " + Math.floor(amount * 1) + " Quarters** from " + taggedUser.username + "#" + taggedUser.discriminator + ".**" + "\n\n**Guild Name:** " + message.guild.name + "\n**Guild ID:** " + message.guild.id + "\n========================");
		}
		await sqlHand.setData(client, `./SQL/guild_data/${guild.id}/members/member_data.sqlite`, config.usersSQLSetter, memberInfo);
	}

	if(amount < 0)
		message.channel.send(message.author + ", you can't remove negative quarters.");
